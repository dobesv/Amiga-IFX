							...
							IF asl
								IF AslRequest(asl,
										[ASL_DIR,		IF id THEN id ELSE '',
										 ASL_FILE,      FilePart(str_info.specialinfo::stringinfo.buffer),
										 NIL, NIL])<>NIL
									-> asl.drawer and asl.file are the strings
									NEW temp[256]
									IF temp
										-> All names should relative to def_dir
										CopyMem(def_dir, temp, StrLen(def_dir))
										
										-> Add all the strings together
										IF AddPart(temp, asl.drawer, 256)
										IF AddPart(temp, asl.file, 256)
						...
						CASE CY_TYPE
							IF act_curr
								SELECT code
									...
									CASE 1 -> "Preloaded Sound"
										act_curr.type := ACT_SOUND
									CASE 2 -> "Speech"
										act_curr.type := ACT_SPEECH
									CASE 3	-> "Executable"
									...
									CASE 6	-> "Exclude"
										act_curr.type := ACT_DUMMY
									CASE 7  -> "Message"
										act_curr.type := ACT_REQUESTER
								ENDSELECT
							ELSE
						...
						CASE STR_DIR
							IF str_dir
								IF curdir
									...
									UnLock(curdir)
								ENDIF
								curdir := Lock(str_dir.specialinfo::stringinfo.buffer, ACCESS_READ)
								CurrentDir(curdir)
							ENDIF
						DEFAULT
...
PROC maingadgets(scr:PTR TO screen, vi, glistptr)
	DEF ng:PTR TO newgadget,
    	...
    	gad := CreateGadgetA(TEXT_KIND, gad, ng,
    		[NIL,				NIL])
		
		-> Create next 5 line(s)
		ng.topedge		:= ng.topedge+ng.height
		ng.gadgettext	:= '© 1997 by Dobes Vandermeer'
		gad := CreateGadgetA(TEXT_KIND, gad, ng, [NIL, NIL])
		ng.topedge		:= ng.topedge+ng.height
		ng.gadgettext	:= 'All rights reserved'
		gad := CreateGadgetA(TEXT_KIND, gad, ng, [NIL, NIL])
		ng.topedge		:= ng.topedge+ng.height
		ng.gadgettext	:= ''
		gad := CreateGadgetA(TEXT_KIND, gad, ng, [NIL, NIL])
		
		-> We'll put a button in the middle
		ng.topedge		:= ng.topedge+ng.height
		ng.gadgettext	:= 'Edit IDS'
		ng.gadgetid		:= BUT_EDITIDS
		gad := CreateGadgetA(BUTTON_KIND, gad, ng, [NIL, NIL])
		...
		ng.height		:= 15
		ng.flags		:= PLACETEXT_ABOVE
		ng.gadgettext	:= 'Select default sounds/files directory:'
		ng.gadgetid		:= STR_DIR
		
		-> Get default directory string		
		IF def_dir=NIL
			NEW def_dir[256]
			IF def_dir=NIL THEN Throw("MEM", ' (for default directory string)')
		ENDIF
		NameFromLock(curdir, def_dir, 256)
		
		-> Finish the job
		str_dir := gad := CreateGadgetA(STRING_KIND, gad, ng,
			[GTST_STRING,		def_dir,
			 GTST_MAXCHARS,		256,
			 NIL,				NIL])
...
PROC idsgadgets(scr:PTR TO screen, vi, glistptr)
	DEF ng:PTR TO newgadget,
		gad:PTR TO gadget,
		topborder
		...
		ng.topedge    := ng.topedge + ng.height + 5
		ng.gadgettext := 'Done'
		ng.gadgetid   := BUT_DONE
		
		but_done := gad := CreateGadgetA(BUTTON_KIND, gad, ng,
			[NIL, NIL])
...
PROC actgadgets(scr:PTR TO screen, vi, glistptr)
	DEF ng:PTR TO newgadget,
    	...
    	cy_type := gad := CreateGadgetA(CYCLE_KIND, gad, ng,
    		[GTCY_LABELS,		['Sound', 'Preloaded Sound', 'Speech', 'Command', 'Other ID', 'Do Nothing', 'Task Exclusion', 'Requester Message',NIL],
    		 GTCY_ACTIVE,		typenum,
    		 NIL,				NIL])
		...
		IF buf
			WHILE done=0
				...
				IF StrLen(list[0])>3
					...
					IF StrCmp(list[0], 'psound')  THEN type := ACT_SOUND
					IF StrCmp(list[0], 'speech')  THEN type := ACT_SPEECH
					IF StrCmp(list[0], 'exec')    THEN type	:= ACT_EXECUTE
					IF StrCmp(list[0], 'other')   THEN type	:= ACT_IFX
					IF StrCmp(list[0], 'exclude')
						type := ACT_DUMMY	
						list[2] := list[1]
						list[1] := 'Task Exclusions'
					ENDIF
					IF StrCmp(list[0], 'message') THEN type := ACT_REQUESTER
					IF StrCmp(list[0], 'defprefsdir')
						IF curdir
							IF olddir=NIL THEN olddir := CurrentDir(NIL)
							UnLock(curdir)
						ENDIF
						
						-> Get directory
						curdir := Lock(list[1], ACCESS_READ)
			...
			REPEAT
				...
				WHILE ifx.list.is_empty()=NIL
					...
					SELECT sel
						...
						CASE ACT_NOTHING
							actname := 'NOTHING'
						CASE ACT_DUMMY	-> Task Exclusion
							actname := 'EXCLUDE'
							idname  := act.name
							act.name := 'This is ignored!'
						CASE ACT_REQUESTER -> Message
							actname := 'MESSAGE'
						DEFAULT
							actname := NIL
					ENDSELECT
...
PROC entrynum(l:PTR TO list, num:LONG) IS l.entrynum(num)
...
->	data := action.name
	...
	SELECT type
		...
		CASE ACT_EXECUTE
			...
			SystemTagList(data, [SYS_INPUT,		temp, 
								 SYS_OUTPUT,	temp,
								 NIL,			NIL])
			Close(temp)
